...
class Settings extends BaseController
{
    ...
    public function kb()
    {
        ...
        if($this->request->getPost('do') == 'submit'){
            ...
            $validation->setRule('kb_attachment', lang('Admin.settings.allowAttachments'), 'required|in_list[0,1]', [
                'in_list' => 'Select a valid option of Allow attachments.'
            ]);
            if ($this->request->getPost('kb_attachment') == 1) {
                $validation->setRule('kb_attachment_number',lang('Admin.settings.numberAttachments'),'required|is_natural');
                $validation->setRule('kb_file_size', lang('Admin.settings.maxUploadSize'), 'required|greater_than[0]');
                $validation->setRule('kb_file_type', lang('Admin.settings.allowedFileTypes'), 'required');
            }
            if($validation->withRequest($this->request)->run() == false){
                $error_msg = $validation->listErrors();
            }else{
                $file_types = explode(',', $this->request->getPost('kb_file_type'));
                $file_types = array_map(function ($e) {
                    $e = trim($e);
                    if ($e != '') {
                        return $e;
                ...
                $this->settings->save([
                    ...
                    'kb_attachment' => $this->request->getPost('kb_attachment'),
                    'kb_attachment_number' => ($this->request->getPost('kb_attachment') == '1' ? $this->request->getPost('kb_attachment_number') : $this->settings->config('kb_attachment_number')),
                    'kb_file_size' => ($this->request->getPost('kb_attachment') == '1' ? $this->request->getPost('kb_file_size') : $this->settings->config('kb_file_size')),
                    'kb_file_type' => ($this->request->getPost('kb_attachment') == '1' ? serialize($file_types) : $this->settings->config('kb_file_type'))
                ]);
                $this->session->setFlashdata('form_success',lang('Admin.settings.updated'));
                return redirect()->to(current_url());
            }
        ...
        if($this->request->getPost('do') == 'submit'){
            ...
            if($validation->withRequest($this->request)->run() == false){
                ...
                $emailLib->updateTemplate([
                    'subject_'.$this->locale => $this->request->getPost('subject'),
                    'message_'.$this->locale => $this->request->getPost('message'),
                    'last_update' => time()
                ], $template->id);
