...
class Emails
{
    ...
    public function sendFromTemplate($template_id, $data, $user_email, $attachments='')
    {
        ...
        $subject = str_replace(array_keys($data), array_values($data), $template->{'subject_'.$locale});
        $message = str_replace(array_keys($data), array_values($data), $template->{'message_'.$locale});
