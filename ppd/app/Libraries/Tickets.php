...
class Tickets
{
    ...
    public function insertCustomField()
    {
        ...
        $customFieldsModel->insert([
            ...
            'topics' => ($request->getPost('topic_list') == '0' ? '' : serialize($request->getPost('topic'))),
            'value' => $values,
    ...
    public function customFieldsFromTopic($topic_id)
    {
        $customFieldsModel = new CustomFields();
        $q = $customFieldsModel->where('topics', '')
            ->orLike('topics', '"' . $topic_id . '"')
            ->get();
        if ($q->resultID->num_rows == 0) {
            return null;
        }
    ...
    {
        $ticket = $this->ticketsModel->select('tickets.*')->table('tickets')
            ->join('users as u', 'u.id=tickets.user_id')
            ->join('topic as t', 't.id=tickets.topic')
            ->join('category as c', 'c.id=tickets.category', 'left')
            ->join('staff as s', 's.id=tickets.staff_id', 'staff')
            ->join('priority as p', 'p.id=tickets.priority_id')
            ->get();
        $custom_vars = $ticket->getResult();
        // $cust = json_decode(json_encode($custom_vars), true);
        $ticket->freeResult();
        return $custom_vars;
    }
